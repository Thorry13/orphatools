...
```
For all of these functions, it is sometimes more useful to get directly an edgelist or an equivalent graph, 
...
#### Analyze relationships on multiple codes
When several ORPHAcodes are submitted, you can either analyze what they have in common or 
merge branches from their own tree to get an extended classification.
...
If you analyze a set of ORPHAcodes, you may want to quickly build a graph to be able to visualize
relationships between them, or check that your list is complete. `merge_branches` and `complete_family` 
were implemented in this perspective. 

`merge_branches` just finds ancestors and/or descendants from all given ORPHAcodes and connects the whole list.
```{r merge_branches, results='hold'}
orpha_codes = c('303', '305', '595356')
...
Instead `complete_family` first needs to know which level to reach (`max_depth` argument)
before finding all descendants of the found root ancestors.
...
```
`complete_family` with `max_depth=0` is equivalent to `merge_branches` with `direction='down'`.
