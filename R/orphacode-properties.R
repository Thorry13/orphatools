...
load_raw_nomenclature = function(){
  version = getOption('orphatools_nomenclature', default_nom_version())
  extdata_path = system.file('extdata', package='orphatools')
  nomenclature_path = file.path(extdata_path, 'nom_data', version, 'nomenclature.RDS')

  if(file.exists(nomenclature_path))
    df_nomenclature = readRDS(nomenclature_path)
  else
    stop(simpleError(
'Loading of nomenclature data failed. Internal files might be broken.
See `orphatool_options`, `add_nomenclature_pack` or consider reisntalling orphatools package.'))
...
translate_orpha_concepts = function(X, cols=NULL){
  ...
  extdata_path = system.file('extdata', package='orphatools')
  dict_path = file.path(extdata_path, 'dicts', paste0(dict, '.csv'))
...
See `orphatool_options` or consider reisntalling orphatools package.'))
